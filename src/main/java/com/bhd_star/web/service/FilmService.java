...
public class FilmService {
                ...
                if (!file.isEmpty()) {
                    try {
                        ...
                        if (rootNode.has("data")) {
                            String imageUrl = rootNode.path("data").path("image").path("url").asText();
                            String deleteUrl = rootNode.path("data").path("delete_url").asText();
    ...
    public String deleteFilm(String filmId) {
        // Tìm film theo ID
        Film film = filmRepository.findById(filmId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy Film"));
